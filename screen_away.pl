use Irssi; use strict; use FileHandle; use vars qw($VERSION %IRSSI); $VERSION = "0.9.7.2"; %IRSSI = (
    authors => 'Andreas \'ads\' Scherbaum <ads@wars-nicht.de>',
    name => 'screen_away',
    description => 'set (un)away and login/logout from facebookchat, if screen is attached/detached',
    license => 'GPL v2',
    url => 'none', );
# screen_away irssi module
#
# written by Andreas 'ads' Scherbaum <ads@ufp.de>
# modded by Riku 'Riqpe' Silvola <rikoupe@gmail.com>
#
# changes:
#  03.03.2011 disconnects and reconnects from facebookchat, default user jabber, default network bitlbee
#
#  07.02.2004 fix error with away mode
#             thanks to Michael Schiansky for reporting and fixing this one 07.08.2004 new function for changing nick on away 24.08.2004 
#  fixing bug where the away nick was not storedcorrectly
#             thanks for Harald Wurpts for help debugging this one 17.09.2004 rewrote init part to use $ENV{'STY'} 05.12.2004 add patch 
#  for remember away state
#             thanks to Jilles Tjoelker <jilles@stack.nl> change "chatnet" to "tag" 18.05.2007 fix '-one' for SILC networks
#
#
# usage:
#
...
#
# /set screen_away_active ON/OFF/TOGGLE /set screen_away_repeat <integer> /set screen_away_message <string> /set screen_away_window 
# <string> /set screen_away_nick <string> /set screen_away_bitlbee_server_name <string> /set screen_away_fabo_active ON/OFF/TOGGLE 
# /set screen_away_fabo_name <string>
#
# active means, that you will be only set away/unaway, if this
#   flag is set, default is ON repeat is the number of seconds, after the script will check the screen status again, default is 5 seconds 
# message is the away message sent to the server, default: not here ... window is a window number or name, if set, the script will switch
#   to this window, if it sets you away, default is '1' nick is the new nick, if the script goes away will only be used it not empty
#
# normal you should be able to rename the script to something other than 'screen_away' (as example, if you dont like the name) by simple 
# changing the 'name' parameter in the %IRSSI hash at the top of this script variables
...
# search for socket normal we could search the socket file, ... if we know the path but so we have to call one time the screen executable 
# disable locale the quotes around C force perl 5.005_03 to use the shell thanks to Jilles Tjoelker <jilles@stack.nl> for pointing this 
# out
...
Irssi::settings_add_bool('misc', $IRSSI{'name'} . '_fabo_active', 1); 
Irssi::settings_add_str('misc', $IRSSI{'name'} . '_fabo_name', "jabber"); 
Irssi::settings_add_str('misc', $IRSSI{'name'} . '_bitlbee_server_name', "bitlbee");
# init process
screen_away();
# screen_away()
#
    ...
    if ($away == 1 and $away_status != 1) {
      ...
      foreach $server (Irssi::servers()) {
        if (!$server->{usermode_away}) {
         ...
         if (Irssi::settings_get_bool($IRSSI{'name'} . '_fabo_active') == 1) {
		my $faboname = Irssi::settings_get_str($IRSSI{'name'} . '_fabo_name');
		$server->command("quote privmsg &bitlbee :account $faboname off") if ((!$server->{usermode_away}) && ($server->{tag} eq Irssi::settings_get_str($IRSSI{'name'} . '_bitlbee_server_name')));
		}
      ...
      foreach $server (Irssi::servers()) {
        ...
        if (Irssi::settings_get_bool($IRSSI{'name'} . '_fabo_active') == 1) {
               my $faboname = Irssi::settings_get_str($IRSSI{'name'} . '_fabo_name');
		$server->command("quote privmsg &bitlbee :account $faboname on") if (($server->{usermode_away}) && ($server->{tag} eq Irssi::settings_get_str($IRSSI{'name'} . '_bitlbee_server_name')));
	}
